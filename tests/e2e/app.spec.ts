...
test('opens first screen and switches modes', async ({ page }) => {
  await page.goto('/');
  await expect(page.getByRole('button', { name: /Google 계정으로 시작하기/ })).toBeVisible();
  await page.getByRole('button', { name: /게스트 모드로 즉시 시작하기/ }).click();
  await expect(page.getByRole('heading', { name: /미팅 정보 설정/ })).toBeVisible();
  await page.getByRole('button', { name: /회의 모드/ }).click();
  await expect(page.getByRole('button', { name: /회의 모드/ })).toHaveClass(/selected/);
